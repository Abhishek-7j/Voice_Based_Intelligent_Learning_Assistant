...
def get_history(conv_id, user_id=None):
    conn = get_db_connection()
    if user_id:
        rows = conn.execute(
            "SELECT m.role, m.content FROM messages m JOIN conversations c ON m.conversation_id = c.id WHERE m.conversation_id = ? AND c.user_id = ? ORDER BY m.timestamp ASC",
            (conv_id, user_id)
        ).fetchall()
    else:
...
def delete_conversation(conv_id, user_id=None):
    conn = get_db_connection()
    if user_id:
        conn.execute("DELETE FROM messages WHERE conversation_id IN (SELECT id FROM conversations WHERE id = ? AND user_id = ?)", (conv_id, user_id))
